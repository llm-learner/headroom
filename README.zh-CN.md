...
- **收进任务栏。** 一个小环形托盘图标，点击才展开用量卡片。直接读取本地账本，不用浏览器标签页，也不占桌面位置。
...
任务栏右侧会出现小环形托盘图标，可能在隐藏图标的箭头里。点击展开用量卡片，悬停查看剩余百分比。这一步只读，不会开启自动扣点。如果需要在终端看启动报错，用 `python` 替换 `pythonw`。
...
- **单击**托盘图标展开 / 收起；卡片右上角的 `−` 和 Escape 也可以收起。悬停图标显示剩余百分比。
...
托盘和菜单栏都用内置的环形仪表盘 `assets/menubar-icon.png`。macOS 的菜单栏有明暗两种外观，
深色时改用 `assets/menubar-icon-dark.png` —— 否则深蓝色那段弧会消失。两张都按 72px 出图，
这是 Retina 屏幕上 18pt 槽位需要的尺寸。

把 `HEADROOM_ICON` 指向任意 PNG 即可换成自己的图标，或者放到 `~/.headroom/icon.png`
以免去环境变量。旁边有同名的 `-dark` 文件时，深色菜单栏会自动用它；没有的话会在运行时
把浅色图标的深色部分提亮。一个文件都找不到时，headroom 会自己画一个「粗 H + 环形刻度」。
